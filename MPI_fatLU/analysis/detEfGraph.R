...
colnames(GraphTable) <- c("N", "Processos", "Eficiencia")
...
for (M in MachineNumber[-1]) {
  for (I in NSize) {
    Value <- GraphTable[(GraphTable$N == I & GraphTable$Processos == M), "Eficiencia"]

    GraphTable[(GraphTable$N == I & GraphTable$Processos == M), "Eficiencia"] <-
      GraphTable[(GraphTable$N == I & GraphTable$Processos == 1), "Eficiencia"] / 
      (Value * M)
    print(paste(GraphTable[(GraphTable$N == I & GraphTable$Processos == 1), "Eficiencia"], "/", Value, "*", M));
  }
}
...
ggplot(GraphTable, aes(x=Processos, y=Eficiencia, color=N)) +
  geom_point(size=5) +
